...
class GripPointerModel extends Object3D {
  ...
  _onDisconnected() {
    ...
    if (this._pointerLine && this._pointerLine.material)
      this._pointerLine.material.dispose();
    if (this._pointerLine && this._pointerLine.geometry)
      this._pointerLine.geometry.dispose();
  ...
  createPointer() {
    const lineMaterial = new LineBasicMaterial({
      ...
      pointerLine = (this._pointerLine = new Line(
        new BufferGeometry(),
        lineMaterial,
      )),
      lineColor = new Color(POINTER_COLOR);
    pointerLine.geometry.setAttribute(
      'position',
      new Float32BufferAttribute([0, 0, 0, 0, 0, -this._lineDistance], 3),
    );
    pointerLine.geometry.setAttribute(
      'color',
      new Float32BufferAttribute(
        [lineColor.r, lineColor.g, lineColor.b, 0, 0, 0],
        3,
      ),
    );
  ...
  checkIntersections(objects, recursive = false) {
    if (this._raycaster) {
      ...
      if (intersections.length > 0) {
        ...
        this._cursorObject.position.copy(
          direction.multiplyScalar(CURSOR_MAX_DISTANCE),
        );
      }
    }
  }
